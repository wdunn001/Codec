...
The wire reductions are smaller than the headline 67× to 1404× we see on raw
token streams from inference engines. **This is expected at this layer:**

- MCP responses are already small JSON envelopes (~800 B to 1.8 KB typical).
...
- The **deep wins live in the Codec stack we couldn't run today**:
  - **`msgpack-both + gzip`** (HTTP 415 on the running image: fixed in
    ...
    bench suggests 5 to 8× on `tools/list`.
...
itself dominates**: 3.6 to 3.8 s per call (the Playwright MCP server returning
...
- `tools/list` (19.8 KB, all metadata): 1.12× reduction with msgpack alone,
  and the projected 5 to 8× with gzip is meaningful for clients that re-list
  on every connection.
- `tools/call` on the four `playwright_get|put|patch|delete` tools that
  actually returned ~1.4 KB JSON bodies in 70 to 425 ms: 1.07× reduction,
  again with gzip projected to push into 3 to 5×.
