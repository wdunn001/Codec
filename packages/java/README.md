...
while (frames.hasNext()) {
    CodecFrame frame = frames.next();
    forwardCodecFrame(nextAgent, frame.ids(), frame.done());   // pass long[] straight on
}
```
...
Pre-tokenizers split at whitespace. `Translator` buffers partial words until a safe boundary arrives as a result. For analysis-only use, `Translator.staticTranslationTable(A, B)` gives a context-free `id_A → ids_B` lookup.
...
- **BPE merge ordering**: greedy by priority rather than left-to-right. Matches HuggingFace `tokenizers` reference behavior. Test fixture verifies this explicitly with `[a b c]` + merges `["b c", "a b"]`: priority-correct yields `[0, 4]` (= `a` + `bc`); naive left-to-right would yield `[3, 2]` (= `ab` + `c`).
...
`MapLoader` requests `Accept-Encoding: gzip` and decompresses transparently when the server uses it. The JDK's `HttpClient` doesn't natively decompress brotli/zstd. The request omits them for those and falls back to identity.
