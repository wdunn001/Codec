 ...
 * regex shapes (canonicalized whitespace). They return `null` for anything
 ...
 */
// `\p{N}` with no quantifier matches ONE digit per regex iteration: the
// engine then re-enters the alternation. Digit runs therefore come out one
// digit at a time. That is modelled as max_run=1.
