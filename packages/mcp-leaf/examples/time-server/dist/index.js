...
function convertTime(args) {
    ...
    try {
        ...
        return (`${srcFmt.format(srcDate)} ${source_timezone}  →  ` +
...
async function main() {
    ...
    if (mapUrl && mapHash) {
        try {
            ...
            console.error(`[codec-time-leaf] WARN: failed to load Codec map ${mapUrl}: ${e.message}`);
            console.error('[codec-time-leaf] continuing without leaf-mode tokenization');
        }
    }
    else {
        console.error('[codec-time-leaf] leaf-mode disabled: set CODEC_MAP_URL + CODEC_MAP_HASH to enable');
    }
    // ── tools/list ─────────────────────────────────────────────────────
        ...
        try {
            switch (req.params.name) {
                case 'get_current_time':
                    text = getCurrentTime(args);
                    break;
                case 'convert_time':
                    text = convertTime(args);
                    break;
                default:
...
main().catch((e) => {
    console.error(`[codec-time-leaf] fatal: ${e.message}`);
    process.exit(1);
});
