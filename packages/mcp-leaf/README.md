...
> **Wire shape change in v0.3.2.** Earlier v0.3.0 / v0.3.1 builds emitted Codec metadata as a SIBLING content block (`{ type: "_codec_meta", map_id, ids }`). The MCP SDK's `ContentBlockSchema` is a strict discriminated union over `text | image | audio | resource | resource_link`. The sibling form crashed time-server itself as a result, with `MCP error -32602: Invalid tools/call result` before the result ever left the process. v0.3.2 moves the payload onto the text block's per-block `_meta` field: a first-class MCP spec slot that the SDK validator passes through. The reader-side helpers (`readCodecMeta`, `takeIds`, `stripCodecMeta`) accept BOTH wire shapes for back-compat with results from older Codec-aware tools.
...
Compare the legacy path. It logs:

```
[Codec][shim] tokenizing tool result for vocab 0549cbec9d45…: leaf-mode MCP server would skip this.
```
