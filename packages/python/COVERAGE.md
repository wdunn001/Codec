...
| `server/latent_frame.py`|  0%  | server-side encoder, exercised by integration tests on the lab (not the per-package pytest) |

## Intentionally uncovered

- `server/latent_frame.py` (0%) — server-side latent-frame encoder. Tested
  via `packages/bench/` lab integration runs (`comfyui` / `diffusers`
  containers), not the per-package pytest. Local `pytest` skips it
