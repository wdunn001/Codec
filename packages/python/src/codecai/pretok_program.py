...
The v1 op-set portion of this module is verified against the C runtime
...
recovery time the TypeScript interpreter's whitespace class disagreed with
C and Rust on two code points (native JS ``\\s`` versus
``\\p{White_Space}``; fixed in commit ``79e93ec``). TypeScript's metaspace
splitter still diverges from C and Rust on how a run of non-space-or-tab
whitespace (e.g. consecutive newlines) is pieced. Pinning this file's tests
to the old TypeScript behavior would have baked those bugs into the spec.
See ``packages/python/tests/test_pretok_program.py`` for the in-repo
...
The v2 stage portion is verified piece by piece against the TypeScript
reference, which is the only other client that executes v2 today. The
corpora were golden, combinatorial-stress, and real code and markdown.
...
# U+001F, the information-separator control characters. Those carry
...
# correct and fast. No fallback is needed at all.
