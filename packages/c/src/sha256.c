...
static void sha256_transform(codec_sha256_ctx *ctx, const uint8_t data[]) {
...
void codec_sha256(const uint8_t *data, size_t len, uint8_t hash[CODEC_SHA256_BLOCK_SIZE]) {
    codec_sha256_ctx ctx;
    codec_sha256_init(&ctx);
    codec_sha256_update(&ctx, data, len);
    codec_sha256_final(&ctx, hash);
}
