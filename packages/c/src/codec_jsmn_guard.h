 ...
 * yields two tokens: an OBJECT of size 1 and a STRING. No JSMN_ERROR_PART
 * is raised, because the terminal check only looks for tokens left with
 * `end == -1` and both of these are closed.
 *
 * Every token walker in this library assumes the opposite: that an OBJECT
 * of size N is followed by exactly 2N tokens, and an ARRAY of size N by
 ...
 * That is a heap overread, and in map.c and codec_version_signaling.c the
 * garbage `start` / `end` it produces is then used as an offset and length
 * into the JSON buffer, which turns it into a disclosure primitive.
 *
 * Rather than bound every one of the ~15 read sites, validate the shape
 * once up front. If a walk of the whole token forest consumes exactly
 ...
 * under JSMN_HEADER, so a shared non-inline symbol would need a fourth
 ...
 * vendored jsmn (the `:` sets toksuper to the key, so the value increments
 * it), and the accounting below deliberately ignores that: the walkers
 * model a pair as "key token plus one subtree", so only OBJECT and ARRAY
 * contribute to the pending count.
 */
