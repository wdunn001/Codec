...
exports.decodeMsgpackStream = decodeMsgpackStream;
exports.decodeProtobufStream = decodeProtobufStream;
exports.decodeProtobufFrame = decodeProtobufFrame;
/**
...
function decodeStream(stream, format = 'msgpack') {
    return format === 'msgpack' ? decodeMsgpackStream(stream) : decodeProtobufStream(stream);
}
// ── MessagePack ───────────────────────────────────────────────────────────────
async function* decodeMsgpackStream(stream) {
    for await (const value of (0, msgpack_1.decodeMultiStream)(stream)) {
        const frame = value;
...
// ── Protobuf ──────────────────────────────────────────────────────────────────
async function* decodeProtobufStream(stream) {
