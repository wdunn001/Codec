...
> | **Latents** (diffusers / ComfyUI) | **3.9×** int4 vs raw, ~5 to 10× vs JPEG | `wdunn001/codec-diffusers:v0.3.4` |
>
...
| Python | [`codecai`](https://pypi.org/project/codecai/) | PyPI 0.5.0 | Detokenizer · BPETokenizer · ToolWatcher · Translator · stream decoders · pretok-program runtime · `SafetyPolicyDescriptor` + `discover_safety_policy` (v0.4) |
...
Measured on the lab's AMD EPYC 8124P + gcc:13. Prior README claim (0.61 ns/token, ~100×) was from an un-documented CPU/compiler combination; the **speedup ratio remains ~26-100× in ToolWatcher's favour** depending on the host. That is the marketing-relevant number.
...
When agent A's output feeds agent B as a prompt and the two models have different vocabs, decode-then-reencode through text. The wire itself carries no text:
...
**Path B is optional.** It accepts `application/x-msgpack` or `application/x-protobuf` request bodies, saving 2 to 3× bandwidth on huge prompts (>50K tokens) where the JSON `[int, int, ...]` array balloons. Same streaming output as Path A.
...
Browsers handle decompression transparently in `fetch()`. `@codecai/web` requires zero changes to consume compressed streams.
...
| 32 to 256              | **br**        | gzip          | brotli's static dictionary beats deflate on small structural payloads; per-chunk flush regression removed in v0.4.1 |
| 512                    | **gzip**      | br            | by 512 tokens gzip's sliding window has caught up; br's per-stream header amortises away |
| 1024 to 2048           | **dict-zstd** | gzip          | for cooperative content, dict-zstd opens a 1.5 to 2× lead over gzip as the stream grows |
...
| 32 to 2048 | **br wins every size** | gzip a close second |
...
Works for any bursty small-frame streaming workload (SSE, gRPC-Web text, log streams, telemetry), Codec included.
...
All bench output is deterministic given a fixed RNG seed. If a number in this README looks off, regenerate it from `packages/bench` and PR a fix. These are claims about software.
...
- **Pretok program runtime in .NET**: has `\p{L}` support today. The regex path works fine as-is. Porting the program runtime gives roughly 10 to 30% encode-startup speedup. Python's port landed in this same pass.
