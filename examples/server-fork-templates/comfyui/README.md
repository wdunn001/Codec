...
The fork's `app/` directory then contains the codec module sitting alongside ComfyUI's existing app code. No changes to ComfyUI's plugin loader or `custom_nodes/` mechanism are needed: this is purely a core extension.
...
The current template's `/v1/images/generations` endpoint serves the protocol surface; the workflow-integrated version will live alongside it, without replacing it.
...
The shared `latent_frame.py` keeps the wire-format math identical. That is the contract that matters. Server-framework differences are local concerns.
