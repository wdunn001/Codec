...
Unlike the per-vulnerability references in files 01–06, this file does NOT enumerate CVEs — the checklist is an action-items document, not a vulnerability class. Each P0/P1/P2 item in the checklist already cross-references its motivating threat-model section (which has the CVEs).
...
- Less directly applicable to Codec (which is a transport, not a model) but informs the "supervisor pass" pattern in checklist P2 #18.
...
Items 1–17 of [`../07-codec-client-checklist.md`](../07-codec-client-checklist.md) trace back to specific threat-model sections in files 01–06. Each P0/P1 item has an "(implementation sketch / verification step)" pair so the action is independently testable. This file's purpose is to make the **structure** of the checklist defensible against external review — when a security board asks "where did you get this list," the answer is the union of OWASP LLM Top 10 + OWASP MCP Top 10 + NIST AI RMF + MITRE ATLAS + the specific CVEs catalogued in files 01–06 of this directory.
