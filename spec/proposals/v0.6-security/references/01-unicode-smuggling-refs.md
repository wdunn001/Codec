...
**Status:** no specific CVE: this is a vulnerability class, tracked across many products.
...
**Vendor responses:**
- OpenAI, Anthropic, and Google have all silently added tag-block stripping at API ingestion for their flagship models over 2024 to 2025. Detection coverage at platform-rendering surfaces (X, LinkedIn, GitHub) is inconsistent.
...
- LLM-specific application: Simon Willison's blog (https://simonwillison.net) has multiple posts (2023 to 2026) documenting use against ChatGPT and Claude.
...
**Threat model significance:** carrier-emoji variation-selector chains are a high-capacity covert channel. Single VS characters are legitimate (emoji presentation, CJK font variants). A blanket strip is hostile as a result. Defense pattern: strip *runs* of length ≥ 2 (per [`../01-unicode-smuggling.md`](../01-unicode-smuggling.md) §3).
...
- https://github.com/google-deepmind/synthid-text: adjacent tooling for output watermarking, in the same problem space though not glitch-token specific

**LLM-application angle:** model-specific. Per-model glitch-token lists belong in tokenizer-map metadata (see [`../02-wire-protocol-attacks.md`](../02-wire-protocol-attacks.md) §2), kept out of core Codec encoding logic entirely. New frontier models have been more carefully trained and have fewer glitch tokens; older / specialized models still have catalogued issues.
...
- **October 2025 / early 2026 research:** variation-selector sequences achieving **72 to 100% attack success rate** in controlled experiments on open-source models. Referenced widely in Q1 2026 defensive-tooling vendor blogs.
