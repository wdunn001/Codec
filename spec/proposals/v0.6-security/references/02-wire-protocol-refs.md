...
**LLM-application angle:** Codec uses length-prefixed framing per `spec/PROTOCOL.md`. Reference parsers MUST reject on mismatch; truncate-and-continue is not acceptable.
...
- Multiple SaaS tenant isolation bugs disclosed 2023 to 2025 with various CVE assignments. Best catalog: https://github.com/0xRadi/OWASP-Web-Checklist
...
**LLM-application angle:** prompt-prefix caching for cost optimization is becoming standard. Cache keys MUST include the trust principal (authenticated user/tenant) alongside the content hash, or attacker-primed entries leak to legitimate users.
...
- **PromptCache attacks** documented academically in 2024 to 2025 but no production CVE yet.
...
**Operational lesson:** these 2026 cases set a new bar: disclosure-to-exploit windows are now measured in hours. For Codec releases, the recommended pre-publish security gate (see [`../07-codec-client-checklist.md`](../07-codec-client-checklist.md) crosscutting requirements) must assume a similar weaponization timeline once vulnerabilities are disclosed.
