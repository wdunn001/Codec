...
Some classes of LLM exploitation operate not on a single message but across the conversation as a whole: many fake examples of compliance to override safety training (many-shot jailbreak), gradual topic escalation across turns (crescendo), eviction of system prompts via context pressure, prefilling of the assistant turn to steer output. These attacks largely live above the wire protocol — Codec carries the bytes but can't reason about whether the conversation is being manipulated.

What v0.6 CAN do: define wire-level **per-turn metadata** that lets server-side and client-side guardrails apply differential trust per turn, and support **conversation-supervisor** patterns where a second model re-evaluates the conversation periodically.
...
- **Re-validate assistant-turn content** against safety policy on ingestion: if a user-provided conversation contains "assistant" turns with policy-violating content, reject the entire conversation rather than continuing it.
...
**Defense.**
- **Trajectory-aware safety:** safety evaluation considers the conversation arc, not just the latest turn. Implementations exist as separate "trajectory monitors."
...
- **Hard conversation-length cap** for sensitive applications; force a new conversation rather than slide the window.
...
**Mechanism.** Claude's API supports an `assistant_prefill` parameter — the developer pre-populates the start of the assistant's response, and the model continues from there. If user input flows into this parameter, the user controls the start of the model's reply, which can steer it past safety refusal:
...
2. **Recommended client behavior** documented in `spec/PROTOCOL.md`:
   ...
   - Reject `user_provided` assistant turns containing policy-violating content rather than continue from them.
...
The right v0.6 posture is "the wire format supports the metadata that good defenses need; the reference implementation demonstrates the recommended pattern; production deployments choose their own safety stack." Codec is the substrate, not the safety system.
