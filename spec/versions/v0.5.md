...
> the spec as it stands at v0.5 — currently the latest version, so
> [`spec/PROTOCOL.md`](../PROTOCOL.md) (the navigation index)
> resolves here. The "Open questions" sections at the bottom are
...
so the suppression case is a v0.0/v0.1 baseline and is rare.
...
when the client doesn't ask — that's an operator deployment choice,
not a wire concern. The constraint per § Capabilities are opt-on at
...
the way CORS solves "non-CORS browser hits a CORS-protected origin" —
the server emits a *structured* incompatibility signal that v0.X+
clients parse and surface as an upgrade prompt, and older clients still
see something they can render as a useful error rather than a bare 5xx.
...
for the case where compatibility is **structurally impossible** —
e.g. the server is configured to *require* safety-policy enforcement
...
Request side (added in v0.4 — older clients omit this header, which
servers treat as "v0.0 baseline; assume the client speaks no v0.4
features"):
...
(reserved, not yet emitted until session protocol lands; see
...
This is the CORS-preflight analog — opportunistic, not required. The
...
`error` field, which old clients can show as an opaque error
...
between, and the wire reflects whichever one is active. A server
...
and only to clients that speak them.
...
exactly the v0.3 wire it would have emitted if the code were v0.3
— zero v0.4 bytes touch the wire, regardless of client version.
...
handshake itself (the `Codec-Client-Version` request header, and the
...
behaviors, so servers must not bet on graceful tolerance).
...
wire are (a) the 426 response when refused outright, and (b)
nothing else.
...
added to HELLO and each READY field landed as optional, so older
clients kept working) but the spec had no normative rule preventing
...
| New frame type at an unused byte (currently 0x00–0x06 used; 0x07+ free)  | ✓        |
...
- `finish_reason: "policy_violation"` is a new value on the open
  ...
  filtered" signal, which is degraded but not broken.
...
1. **Capability bitmap + OPTIONS-preflight contract — deferred to v0.6.**
   ...
   the [Versioning Policy](#versioning-policy), so getting the v0.4
   region right is gating. Carved out to the v0.6 security + header
   ...
   for now) so v0.6 only swaps the encoding, not the negotiation flow?
...
3. **Discoverable dicts — multi-dict per origin.** v0.5 supports one
   ...
   files? Today the dict is keyed by encoding (msgpack vs protobuf),
   not by model. Multi-tenant deployments may want per-tokenizer dicts
