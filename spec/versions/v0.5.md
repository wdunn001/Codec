...
message CodecFrame {
  ...
  repeated ToolCall tool_calls    = 4;  // server-side ToolWatcher, v0.4.1+
...
**Field 4 was claimed twice and is resolved here.** The v0.5 summary above
described `base_id` as field 4. sglang, vLLM, and llama.cpp have all shipped
`repeated ToolCall tool_calls = 4` since v0.4.1 and serve that definition to
clients from `GET /codec/schema`. The two readings disagree on wire type as
well as on meaning: length-delimited against varint. Shipped code wins.
`tool_calls` keeps field 4. `base_id` moves to field 5.

`ToolCall` shipped in three engines without ever being written into a spec
version. It is normative from v0.5.

Emitters MUST follow proto3 implicit-presence rules for `done`. A field at its
default value is omitted. Writing `done: false` explicitly costs two bytes on
every non-final frame. It is also not what a generated proto3 encoder produces.
...
UI. Tokens emitted before the terminal frame
...
Server response (preference order: zstd > br > gzip > identity):
  ...
  Vary: Accept-Encoding, Codec-Client-Version
```

`Codec-Client-Version` is part of the `Vary` list from v0.4 onward. The
§ Graceful downgrade rule makes the response body depend on the client's
declared version. A v0.3 client and a v0.5 client requesting the same URL
receive different bytes. A shared cache that keys only on `Accept-Encoding`
will hand one of them the other's response. Servers MUST list every request
header that shapes the response, per RFC 9110 § 12.5.5.

`stream_format` is a request **body** field. It cannot appear in `Vary` at
all. A response whose shape depends on it is not cacheable by a shared cache.
Deployments that put a cache in front of a Codec endpoint MUST either send
`Cache-Control: private` or move the format selection into a request header.
...
| (raw uint32, no framing) | 4.0    |       38.5× |

The 4.0 figure is the cost of fixed-width uint32 ids with no framing at all.
It is not a lower bound. Token ids are small integers with a skewed
distribution. Varint forms beat it. `packages/bench/scripts/frame_shape_bench.py`
measures 2.28 bytes per token for LEB128 ids in a msgpack `bin` at 16 tokens
per frame, on real Qwen2.5 ids. `msgpack-delta`, specified in this version,
also beats it. See [`spec/proposals/v0.6-wire-shape.md`](../proposals/v0.6-wire-shape.md)
...
```
Content-Encoding: zstd
Codec-Zstd-Dict:  sha256:29a810f3fbded045d55f1cd4435c7d2959f6dbc9c697dc7fe41fb44bd2e891db
...
```
Codec-Tokenizer-Map: sha256:1df0d6a894b844712979207a0521c3887026f3dde427fb75b2984307a57d797f
```
...
Image and video generation models emit **latent tensors**.
...
**Why the header carries shape and dtype once.** Per-frame metadata adds 30 to 60 bytes per `LatentFrame` versus
...
as they are produced. `LatentFrame.seq` is monotonic. `done=true`
...
against any other pipeline. Mixing dicts across pipelines silently
miscompresses the stream.
...
supported modalities. The client can then fail fast on a stream it
cannot decode.
...
```json
{
  ...
  "map_hash": "sha256:1df0d6a894b844712979207a0521c3887026f3dde427fb75b2984307a57d797f",
...
- `finish_reason: "policy_violation"` is a new value on the open
  ...
  "stream ended with an error" in place of a precise "policy
