...
2. **Per-stack-aware** — picker ships per-engine threshold profiles (`sglang.profile.json`, `vllm.profile.json`, `llamacpp.profile.json`, future `tgi.profile.json`). Each profile carries the engine's measured B/token crossover curve per encoding. Deployments can override the bundled profile with a custom one for their measured workload.
...
8. **TGI as the 4th cross-stack engine.** v0.5 adds Hugging Face
   TGI to the cross-stack matrix ([Task #89]). Open: does TGI's
