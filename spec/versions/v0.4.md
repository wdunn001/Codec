# Codec Protocol Specification: v0.4

> **This is a per-version snapshot.** The wire-format text below is
> the spec as it stands at v0.4, currently the latest version.
> [`spec/PROTOCOL.md`](../PROTOCOL.md) (the navigation index)
> resolves here as a result. The "Open questions" sections at the bottom are
> **living**: when v0.5 ships, items it resolves get marked
> `**Resolved.**` (with strikethrough on the original question)
> here, AND on the v0.2/v0.3 snapshots if v0.5 closes anything from
> them. Per the [Versioning Policy](#versioning-policy) (codified in
> v0.4), every later minor version is wire-additive over this one.
>
...
fabric. v0.4 adds safety-policy negotiation as one more axis on the
existing TLS-style handshake. It also codifies the no-breaking-changes-in-
minor-versions rule (see § Versioning Policy).
...
`Codec-Client-Version` joins the `Vary` list in this version. This is the
version that introduces it. The § Graceful downgrade rule makes the
response body depend on the client's declared version. A v0.3 client and a
v0.4 client requesting the same URL receive different bytes. A shared cache
that keys only on `Accept-Encoding` will hand one of them the other's
response. Servers MUST list every request header that shapes the response,
per RFC 9110 § 12.5.5.

A v0.2 or v0.3 server has no version-dependent shaping. `Vary:
Accept-Encoding` alone remains correct for those versions.
...
4. ~~**Token-ID list authoring tooling.** Operators populate
   ...
   writes the per-tokenizer-id token-sequence arrays?~~ **Resolved
   at v0.5.** See [`spec/versions/v0.5.md` § "Open questions (v0.5)"](v0.5.md#open-questions-v05)
   item 4 for the resolved-direction note.
