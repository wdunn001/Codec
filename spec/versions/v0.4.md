...
- `accept_safety_policies` on HELLO is optional; v0.3 clients omit
  it and v0.4 servers proceed with their deployment-default policy
  (or no enforcement) per the safety-policy negotiation rules.
- `safety_policy_id` and `safety_policy_hash` on READY are optional;
  v0.3 clients ignore them.
